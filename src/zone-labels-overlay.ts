import { World, ClimateZone, Renderer } from './types';

const MIN_REGION_SIZE = 15;

const ZONE_NAMES: Record<number, string> = {
  [ClimateZone.Temperate]: 'TEMPERATE',
  [ClimateZone.Tropical]: 'TROPICAL',
  [ClimateZone.Mediterranean]: 'MEDITERRANEAN',
  [ClimateZone.Desert]: 'DESERT',
};

const ZONE_LABEL_COLORS: Record<number, string> = {
  [ClimateZone.Temperate]: 'hsl(120, 40%, 65%)',
  [ClimateZone.Tropical]: 'hsl(80, 55%, 60%)',
  [ClimateZone.Mediterranean]: 'hsl(30, 55%, 65%)',
  [ClimateZone.Desert]: 'hsl(45, 60%, 70%)',
};

interface ZoneRegion {
  zone: ClimateZone;
  centroidX: number;
  centroidY: number;
}

interface LabelEl {
  el: HTMLElement;
  gridX: number;
  gridY: number;
}

/** BFS flood-fill to find connected components of each climate zone. */
function findZoneRegions(world: World): ZoneRegion[] {
  const W = world.width;
  const H = world.height;
  const visited = new Uint8Array(W * H);
  const regions: ZoneRegion[] = [];

  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      const idx = y * W + x;
      if (visited[idx]) continue;
      const zone = world.grid[y][x].climateZone;

      // BFS
      const queue: number[] = [idx];
      visited[idx] = 1;
      let sumX = 0, sumY = 0, count = 0;
      let qi = 0;

      while (qi < queue.length) {
        const ci = queue[qi++];
        const cy = (ci / W) | 0;
        const cx = ci - cy * W;
        sumX += cx;
        sumY += cy;
        count++;

        for (const [dx, dy] of [[-1, 0], [1, 0], [0, -1], [0, 1]] as const) {
          const nx = cx + dx;
          const ny = cy + dy;
          if (nx < 0 || nx >= W || ny < 0 || ny >= H) continue;
          const ni = ny * W + nx;
          if (visited[ni]) continue;
          if (world.grid[ny][nx].climateZone !== zone) continue;
          visited[ni] = 1;
          queue.push(ni);
        }
      }

      if (count >= MIN_REGION_SIZE) {
        regions.push({
          zone,
          centroidX: sumX / count,
          centroidY: sumY / count,
        });
      }
    }
  }

  return regions;
}

export function createZoneLabelsOverlay(
  mapContainer: HTMLElement,
  renderer: Renderer,
  world: World,
) {
  let visible = false;

  const overlay = document.createElement('div');
  overlay.style.cssText = `
    position:absolute; top:0; left:0; width:100%; height:100%;
    pointer-events:none; z-index:8; overflow:hidden;
  `;
  mapContainer.appendChild(overlay);

  const labels: LabelEl[] = [];

  function buildLabels(w: World): void {
    for (const lbl of labels) overlay.removeChild(lbl.el);
    labels.length = 0;

    const regions = findZoneRegions(w);
    for (const region of regions) {
      const color = ZONE_LABEL_COLORS[region.zone] ?? '#888';
      const name = ZONE_NAMES[region.zone] ?? '?';

      const el = document.createElement('div');
      el.style.cssText = `
        position:absolute; transform:translate(-50%, -50%);
        font-family:monospace; font-size:13px; font-weight:bold;
        text-transform:uppercase; letter-spacing:1px;
        color:${color}; opacity:0.85;
        text-shadow: 0 1px 3px rgba(0,0,0,0.8), 0 0 6px rgba(0,0,0,0.5);
        white-space:nowrap; display:none;
      `;
      el.textContent = name;
      overlay.appendChild(el);

      labels.push({ el, gridX: region.centroidX, gridY: region.centroidY });
    }
  }

  // Initial build
  buildLabels(world);

  function setVisible(show: boolean): void {
    visible = show;
    if (!show) {
      for (const lbl of labels) lbl.el.style.display = 'none';
    }
  }

  function updatePositions(): void {
    if (!visible) return;
    for (const lbl of labels) {
      const screen = renderer.projectToScreen(lbl.gridX, lbl.gridY);
      if (screen) {
        lbl.el.style.left = `${screen.x}px`;
        lbl.el.style.top = `${screen.y}px`;
        lbl.el.style.display = '';
      } else {
        lbl.el.style.display = 'none';
      }
    }
  }

  function rebuild(w: World): void {
    buildLabels(w);
  }

  return { setVisible, updatePositions, rebuild };
}
