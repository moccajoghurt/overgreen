...
/**
 * Terrain Quad (Tropical): 4 terrain pockets (Soil, Hill, Wetland, Arid)
 * in a 2×2 layout, all under Tropical climate. 35×35 pockets separated by 10-cell Rock barriers.
 *
 * Layout:
 ...
 */
export const experimentTerrainQuadTropical: Scenario = (() => {
  const size = 80;
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (quad) {
        cells.push({ x, y, terrain: quad.terrain, elevation: quad.elevation, climateZone: ClimateZone.Tropical });
      } else {
  ...
  return {
    id: 'experiment-terrain-quad-tropical',
    name: '[Exp] Terrain Quad (Tropical)',
    description: '2×2 grid of 4 terrain types (Soil, Hill, Wetland, Arid) under Tropical climate, separated by 10-cell Rock barriers. 35×35 pockets (1,225 cells each).',
    size,
