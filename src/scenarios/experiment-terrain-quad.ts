...
/**
 * Terrain Quad: 4 terrain pockets (Soil, Hill, Wetland, Arid) in a 2×2 layout,
 * all under Temperate climate. 35×35 pockets separated by 10-cell Rock barriers.
 *
 * Layout:
...
export const experimentTerrainQuad: Scenario = (() => {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (quad) {
        cells.push({ x, y, terrain: quad.terrain, elevation: quad.elevation, climateZone: ClimateZone.Temperate });
      } else {
  ...
  return {
    id: 'experiment-terrain-quad',
    name: '[Exp] Terrain Quad',
    description: '2×2 grid of 4 terrain types (Soil, Hill, Wetland, Arid) under Temperate climate, separated by 10-cell Rock barriers. 35×35 pockets (1,225 cells each) for reliable subtype emergence.',
    size,
