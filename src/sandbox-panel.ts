...
export function createSandboxPanel(
  ...
  let currentGenome: Genome = {
    ...
    seedInvestment: 0.5, seedSize: 0.5, defense: 0.5, woodiness: 0.5, waterStorage: 0.5, longevity: 0.5,
  };
...
.sb-slider-group { display: flex; flex-direction: column; gap: 6px; }
  ...
  for (const [name, values] of Object.entries(PRESETS)) {
    ...
    btn.addEventListener('click', () => {
      const g: Genome = { rootPriority: 0.5, heightPriority: 0.5, leafSize: 0.5, seedInvestment: 0.5, seedSize: 0.5, defense: 0.5, woodiness: 0.5, waterStorage: 0.5, longevity: 0.5 };
